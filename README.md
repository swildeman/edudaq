...
|c|channel calMode [A] [B] [C]| `c 1 10.2 2.5 0`|Set channel `channel` to calibration mode `calMode` (no calibration = 0, polynomial = P, or NTC thermistor = T). In P-mode, A, B, and C are the coefficients for the polynomial (A + Bx + Cx^2). In T-mode the coefficients are used in the Steinhart-Hart Equation (A + B ln(RT/R0) + C ln(RT/R0)^3), where the the signal is assumed to come from a voltage divider R0/RT with R0 connected to 5V and the NTC thermistor RT connected to GND. |
